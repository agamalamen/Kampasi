...
$('.tweets-counter').each(function (event) {
    $(this).prop('Counter',0).animate({
        Counter: $(this).text()
    }, {
        duration: 150000,
    ...
    setInterval(function () {
      document.getElementById("twitters").innerHTML= document.getElementById("twitters").innerHTML + "<img style='width: 50px; height: 50px;' src=" + twitterImg +">";
    }, 350);
});

setInterval(function () {
  document.getElementById("all").innerHTML= "<h1 style='position: absolute; color: white; -webkit-text-stroke-width: 3px; -webkit-text-stroke-color: black; font-size: 100px; top: 40%; font-weight: bold; right: 25%; font-family: lato;'>CANDY RUSH</h1>";
  document.body.style.backgroundImage = "url("+ candyRush +")";
}, 150000);

$('input:radio[name="chaperoneRadio"]').change(
    function(){
        if ($(this).is(':checked') && $(this).val() == 'internal') {
            $('#internalChaperone').attr('class', 'form-group');
            $('#externalChaperone').attr('class', 'form-group hidden');
        } else if ($(this).is(':checked') && $(this).val() == 'external') {
          $('#internalChaperone').attr('class', 'form-group hidden');
          $('#externalChaperone').attr('class', 'form-group');
        }
    });
